  ...
  for (i in 1:N.simul.lambda){
    ...
    for (ii in 1:n.test){
      if (ii == 1){
        # warning: square the vol
        # apply the shrinkage
        pred.h.simul[ii] = lambda*(coef.simul["omega"] + train.set.r.temp[n.train]^2*coef.simul["alpha1"] + train.set.vol.temp[n.train]^2*coef.simul["beta1"])
      } else {
        # we observed test.r[ii - 1]
        # apply the shrinkage
        pred.h.simul[ii] = lambda*(coef.simul["omega"] + test.set.r.temp[ii - 1]^2*coef.simul["alpha1"] + pred.h.simul[ii - 1]*coef.simul["beta1"])
      }
    }
...
new.beta.x = (length(param.arr.window[,1]) + 1):(length(param.arr.window[,1])+n.test)
prediction.new.beta = predict(lm.beta1, newdata=data.frame(x=new.beta.x))

new.alpha.x = (length(param.arr.window[,1]) + 1):(length(param.arr.window[,1])+n.test)
prediction.new.alpha = predict(lm.alpha1, newdata=data.frame(x=new.alpha.x))

prediction.regression.regularisation = numeric(n.test) # regularisation with 0.9
prediction.regression.regularisation.two = numeric(n.test) # regularisation with 1.1
prediction.regression.no.regularisation = numeric(n.test) 
lambda.star = 0.90 # prediction.regression.regularisation
lambda.prime = 1.10 # prediction.regression.regularisation.two
for (i in (n.train - rolling.window.size + 1):(n.train - rolling.window.size + n.test)){
  # 8.5 Fit GARCH
  garch_model.bootstrap.window <- garchFit(~arma(1, 0) + garch(1, 1), data = df$logReturn[(i - 1):(i+rolling.window.size - 1)], include.mean = FALSE, trace=F)

  # 8.6 Predict
  cur = garch_model.bootstrap.window@h.t[length(garch_model.bootstrap.window@h.t)]

  prediction.regression.no.regularisation[i - (n.train - rolling.window.size)] = sqrt(cur*garch_model.bootstrap.window@fit$coef[4] + df$logReturn[i - 1]^2*garch_model.bootstrap.window@fit$coef[3])
  
  save.beta = garch_model.bootstrap.window@fit$coef[4]
  save.alpha = garch_model.bootstrap.window@fit$coef[3]
  
  garch_model.bootstrap.window@fit$coef[4] = prediction.new.beta[i - (n.train - rolling.window.size)] + lambda.star*(save.beta - prediction.new.beta[i - (n.train - rolling.window.size)]) # surgically adjust beta, shrink towards regression line
  garch_model.bootstrap.window@fit$coef[3] = prediction.new.alpha[i - (n.train - rolling.window.size)] + lambda.star*(save.alpha - prediction.new.alpha[i - (n.train - rolling.window.size)]) # surgically adjust alpha, shrink towards regression line
  
  prediction.regression.regularisation[i - (n.train - rolling.window.size)] = sqrt(cur*garch_model.bootstrap.window@fit$coef[4] + df$logReturn[i - 1]^2*garch_model.bootstrap.window@fit$coef[3])

  garch_model.bootstrap.window@fit$coef[4] = prediction.new.beta[i - (n.train - rolling.window.size)] + lambda.prime*(save.beta - prediction.new.beta[i - (n.train - rolling.window.size)]) # surgically adjust beta, shrink towards regression line
  garch_model.bootstrap.window@fit$coef[3] = prediction.new.alpha[i - (n.train - rolling.window.size)] + lambda.prime*(save.alpha - prediction.new.alpha[i - (n.train - rolling.window.size)]) # surgically adjust alpha, shrink towards regression line
  
  prediction.regression.regularisation.two[i - (n.train - rolling.window.size)] = sqrt(cur*garch_model.bootstrap.window@fit$coef[4] + df$logReturn[i - 1]^2*garch_model.bootstrap.window@fit$coef[3])
}

combined_df_plot = data.frame(c1 = seq(1, length(prediction.regression.regularisation)), c2 = prediction.regression.regularisation, c3 = test.set.vol.actual, c4 = prediction.regression.no.regularisation, c5 = prediction.regression.regularisation.two)

# Plotting both data sets together
ggplot(combined_df_plot) +
  geom_line(aes(x = c1, y = c2, color = "Predicted Sigma Actual (regularisation)"), linewidth=0.2) +
  geom_line(aes(x = c1, y = c3, color = "Test Set Vol Actual"), linewidth=0.2) +
  geom_line(aes(x = c1, y = c4, color = "Predicted Sigma Actual (no regularisation)"), linewidth=0.2) +
  geom_line(aes(x = c1, y = c5, color = "Predicted Sigma Actual (regularisation two)"), linewidth=0.2) +
  labs(title = "Predicted Sigma Actual vs Test Set Vol Actual",
       x = "Index",
       y = "Value") +
  scale_color_manual(name = "Data", values = c("Predicted Sigma Actual (regularisation)" = "blue", "Test Set Vol Actual" = "red", "Predicted Sigma Actual (no regularisation)"= "green", "Predicted Sigma Actual (regularisation two)" = "pink"))
...
calculate_mse(combined_df_plot$c3, combined_df_plot$c2) # with regularisation (lambda = 0.9)
calculate_mse(combined_df_plot$c3, combined_df_plot$c4) # without shrinkage (lambda = 1.0)
calculate_mse(combined_df_plot$c3, combined_df_plot$c5) # over regularisation (lambda = 1.1)

### 8: Task VIII Prediction Error Interval
ntrials = 100 # number of bootstrap runs
param.arr = matrix(0, nrow = ntrials, ncol = length(current.parameters.estimate))
N.pe = 10
prediction.error.arr = matrix(nrow=n.test, ncol=ntrials*N.pe)
...
for (i in 1:ntrials){
  ...
  # 9.1 Simulate bootstrap for prediction error
  for (ii in 1:N.pe){
    
    ### 9.1: Simulate according to ARMA(1, 0) + GARCH(1, 1) model
    garch.spec.temp = garchSpec(model = list(ar=c(0, param.arr[i,1]), alpha=param.arr[i,3], beta=param.arr[i,4], omega=param.arr[i,2]))
    simulated_garch = garchSim(spec = garch.spec.temp, n = n+2, extended=T)
    
    # ggplot() + geom_line(aes(x=1:length(simulated_garch$garch),y=simulated_garch$garch))
    
    ### 9.2 Fit GARCH model
    garch_model.bootstrap.pe <- garchFit(~arma(1, 0) + garch(1, 1), data = simulated_garch$garch[1:n.train], include.mean = FALSE, trace=F)
    
    current.coef.matrix.pe = garch_model.bootstrap.pe@fit$matcoef
    current.parameters.estimate.pe = current.coef.matrix.pe[,1]
    
    ### 9.3: Simulate according to epsilon_t ~ N(0, sigma_t^2)
    pred.h.simul = integer(n.test)
    pred.sigma.simul = integer(n.test)
    pred.sigma.true = integer(n.test)
    for (iii in 1:n.test){
      if (iii == 1){
        # warning: square the vol
        pred.h.simul[iii] = (current.parameters.estimate.pe["omega"] + simulated_garch$garch[n.train]^2*current.parameters.estimate.pe["alpha1"] + simulated_garch$sigma[n.train]^2*current.parameters.estimate.pe["beta1"])
      } else {
        # we observed test.r[ii - 1]
        pred.h.simul[iii] = (current.parameters.estimate.pe["omega"] + simulated_garch$garch[n.train + iii - 1]^2*current.parameters.estimate.pe["alpha1"] + pred.h.simul[iii - 1]*current.parameters.estimate.pe["beta1"])
      }
      # find true realised vol
      pred.sigma.true[iii] = sqrt((simulated_garch$garch[n.train + iii - 2]^2 +
                                    simulated_garch$garch[n.train + iii - 1]^2 +
                                    simulated_garch$garch[n.train + iii]^2 +
                                    simulated_garch$garch[n.train + iii + 1]^2 +
                                    simulated_garch$garch[n.train + iii + 2]^2)/5)
    }
    
    pred.sigma.simul = sqrt(pred.h.simul)
    prediction.error.arr[,(i-1)*N.pe + ii] = (pred.sigma.simul - pred.sigma.true)
  }
}

prediction.error.5 = numeric(n.test) # prediction interval 5th percentile
prediction.error.95 = numeric(n.test) # prediction interval 95th percentile

for (i in 1:n.test){
  prediction.error.5[i] = sort(prediction.error.arr[i,])[round(5/100*ntrials*N.pe)]
  prediction.error.95[i] = sort(prediction.error.arr[i,])[round(95/100*ntrials*N.pe)]
}

# 9.5 Plot of prediction 
ggplot() + geom_line(aes(x=1:n.test,y=prediction.error.5))
ggplot() + geom_line(aes(x=1:n.test,y=prediction.error.95))

# 9.5
pred.sigma.upper = numeric(n.test)
pred.sigma.lower = numeric(n.test)
for (ii in 1:n.test){
  pred.sigma.upper[ii] = pred.sigma.actual[ii] + prediction.error.95[ii]
  pred.sigma.lower[ii] = pred.sigma.actual[ii] + prediction.error.5[ii]
}

combined_df_plot = data.frame(c1 = seq(1, length(pred.sigma.actual)), c2 = pred.sigma.actual, c3 = test.set.vol.actual, c4 = pred.sigma.upper, c5 = pred.sigma.lower)

# 9.6
ggplot(combined_df_plot) +
  geom_line(aes(x = c1, y = c2, color = "Predicted σ")) +
  geom_line(aes(x = c1, y = c3, color = "Actual σ")) +
  geom_line(aes(x = c1, y = c4, color = "Prediction Interval (95th Percentile)"), linetype = "dashed") +
  geom_line(aes(x = c1, y = c5, color = "Prediction Interval (5th Percentile)"), linetype = "dashed") +
  labs(title = "Predicted σ vs Actual σ",
       x = "Time (in months)",
       y = "σ") +
  scale_color_manual(name = "Data", values = c("Predicted σ" = "blue", "Actual σ" = "green", "Prediction Interval (95th Percentile)" = "black", "Prediction Interval (5th Percentile)" = "black"))
